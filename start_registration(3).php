...
<body>
  ...
  <div class="input-group">
    ...
    <input type="tel" name="phone" id="phone" required class="form-control" placeholder="1234567890" pattern="^\d{6,15}$">
  </div>
  <small class="form-text text-muted">
    Format: 6–15 digits. Country code will be filled automatically.
  </small>
</div>
  ...
  $('#REGION').change(function() {
    ...
    if (regionId) {
      ...
      countrySelect.prop('disabled', true); // Disable until data is loaded
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          countrySelect.prop('disabled', false); // ✅ Enable when data is loaded
          schoolSelect.html('<option value="">Select School</option>')
                      .prop('disabled', true); // ✅ Always reset and disable school
        },
        error: function() {
          $('#country-loading').hide();
  ...
  $('#COUNTRY').change(function() {
    ...
    if (countryId) {
      $('#school-loading').show();
      schoolSelect.prop('disabled', true); // Disable until data is loaded
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $('#school-loading').hide();
          schoolSelect.prop('disabled', false); // ✅ Enable when data is loaded
        },
        error: function() {
          $('#school-loading').hide();

...
LEFT JOIN certifications ON certification_courses.course_certificate = certifications.certification_id WHERE course_id ='$COURSE'"));
 ...
 if(isset($_GET['STATUS'])){
	...
	$action = "topic_course='$COURSE' AND topic_status ='$status' AND topic_visibility='Visible'"; 
 }
 else{
$STATUS ="Active";	
$status = $STATUS; 
							...
							<table class="table-responsive w-full rounded">
                                    <thead>
                                      <tr>
                                        ...
                                        <th class="border w-1/5 px-4 py-2">Course</th>
									...
									while($module_details = mysqli_fetch_array($select_modules)){
									    ...
									    $topic_document =$module_details['topic_document'];
                                           ...
                                           <td class="border px-4 py-2"><?php echo $module_details['topic_title'];?> </td>
                                            ...
                                            if(!empty( $topic_document)){
                                                ?><a href="Open_course_details?COURSE=<?php echo $COURSE;?>&CERTIFICATE=<?php echo $CERTIFICATE;?>&CURRENT=<?php echo $STATUS;?>&STATUS=Active&ID=<?php echo $module_details['topic_id'];?>" class="bg-green-500 cursor-pointer rounded p-1 mx-1 text-white">
                                                       <i class="fas fa-eay-open text-green-500 mx-2"></i> Open course</a><?php
                                            }else{
                                               ?><a href="Open_course_details?COURSE=<?php echo $COURSE;?>&CERTIFICATE=<?php echo $CERTIFICATE;?>&CURRENT=<?php echo $STATUS;?>&STATUS=Active&ID=<?php echo $module_details['topic_id'];?>" class="bg-red-500 cursor-pointer rounded p-1 mx-1 text-white">
                                                       <i class="fas fa-eay-open text-green-500 mx-2"></i>No File</a><?php  
                                            }

        ...
        <div class="w-full max-w-lg">
            <div class="leading-loose">
                <form action="" method="POST" class="max-w-xl m-4 p-10 bg-white rounded shadow-xl">
                    ...
                    <p class="text-gray-800 font-medium">Add New Fee Structure to :<br><strong><?php echo $CLASS_DETAILS['level_name']."/".$CLASS_DETAILS['class_name'];?></strong>&nbsp; </p>
                     <?php
...
$select_student = mysqli_query($conn,"SELECT * FROM `school_payment_settings` WHERE spayment_class='$CLASS' AND `spayment_description`='$spayment_description' AND  `spayment_term`='$setting_term' AND `spayment_year`='$setting_year' AND spayment_school='$school_ref'");
...
?><div class="bg-red-500 mb-2 border border-red-300 text-white px-4 py-3 rounded relative" role="alert">
	...
	$insert= mysqli_query($conn,"INSERT INTO school_payment_settings 
	(spayment_id, spayment_school, spayment_level, spayment_class, spayment_description, spayment_amount, spayment_country, spayment_region, spayment_term, spayment_year) VALUES 
	(NULL, '$school_ref', '$class_level1', '$CLASS', '$spayment_description', '$spayment_amount', '$class_country1', '$Country_region1', '$setting_term', '$setting_year')");
if($insert){
?><div class="bg-green-500 mb-2 border border-red-300 text-white px-4 py-3 rounded relative" role="alert">
                                    <strong class="font-bold">Success !!!!</strong>
                                    <span class="block sm:inline">This Description (<?php echo  $spayment_description ;?>)  Has been Added</span>
                                    <span class="absolute top-0 right-0 px-4 py-3">
...
?><div class="bg-red-500 mb-2 border border-red-300 text-white px-4 py-3 rounded relative" role="alert">
                    ...
                    <div class=""> 
                        ...
                        <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="spayment_description" name="spayment_description"  type="text" required >
                    </div>
					 
                    <div class=""> 
                        <label class="block text-sm text-gray-600" for="firstname">Amount</label>
                        <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="spayment_amount" name="spayment_amount"  type="number" required >
                    </div>
                    ...
                    <div class="mt-4">
                        <button type="submit" name="Update" class="px-4 py-1 text-white font-light tracking-wider bg-green-500 rounded">Add Fee Structure</button>
                    </div>
                </form>
            </div>
        </div>

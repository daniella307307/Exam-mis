 ...
 if(isset($_GET['STATUS'])){
	 ...
	 if($STATUS=="Active"){
		$status ="Active"; 
		$action = "room_status ='Active' AND room_school='$school_ref' AND  (class_country='0' OR class_country='$user_country')"; 
	 }
	 else{
		$status ="Inactive";  
		$action = "room_status !='Active' AND room_school='$school_ref' AND  (class_country='0' OR class_country='$user_country')"; 
...
$STATUS ="Active";	
$status = $STATUS; 
$action = "room_status ='$status' AND room_school='$school_ref'";
 }
 $select_user = mysqli_query($conn,"SELECT * FROM school_class_rooms
...
LEFT JOIN  schools  ON school_class_rooms.room_school = schools.school_id WHERE $action");
 ?>
                            ...
                            <div class="p-3">
                                <table class="table-responsive w-full rounded">
									...
									while($users_details = mysqli_fetch_array($select_user)){
										$levels =$users_details['room_level'];
										$classes =$users_details['room_class'];
										$room_school=$users_details['room_school'];
									 ...
									 ?><tr>
                                            <td class="border px-4 py-2"><?php echo $users_details['room_class'];?></td> 
											<td class="border px-4 py-2"><?php echo $users_details['class_name'];?></td> 
                                           <td class="border px-4 py-2">
                                               <?php echo $students_n;?>
                                             <?php
										/* if($students_n>0){
											?> <a href="students_per_class_room?CLASS=<?php echo $users_details['room_class'];?>" class="bg-green-500 cursor-pointer rounded p-1 mx-1 text-white">

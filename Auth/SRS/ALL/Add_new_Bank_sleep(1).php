...
?><div class="bg-red-500 mb-2 border border-red-300 text-white px-4 py-3 rounded relative" role="alert">
	...
	$insert= mysqli_query($conn,"INSERT INTO bank_sleeps (sleep_id, sleep_bank, sleep_school, sleep_country, sleep_region, sleep_no, sleep_amount_usd, sleep_amount_local, sleep_date, sleep_document, sleep_status) VALUES 
	                                                       (NULL, '$bank_id', '$school_ref', '$bank_country', '$bank_region', '$sleep_no', '$sleep_amount_usd', '$sleep_amount_local', '$sleep_date', 'document', 'Waiting')");
if($insert){
...
?><div class="bg-red-500 mb-2 border border-red-300 text-white px-4 py-3 rounded relative" role="alert">
					...
					?>    
					 ...
					 <div class="w-full md:w-2/2 px-3 mb-6 md:mb-0">
                            ...
                            <div class="relative">
                                 <select name="bank_id" id="country"  onChange="getstate(this.value);" class="block appearance-none w-full bg-grey-200 border border-grey-200 text-grey-darker py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-grey" required>
                  ...
                  <center>  <div class="mt-4">
                        <button type="submit" name="Update" class="px-4 py-1 text-white font-light tracking-wider bg-green-500 rounded">Add received Amount in the system </button>
                    </div> </center>
                </form>
            </div>
        </div>

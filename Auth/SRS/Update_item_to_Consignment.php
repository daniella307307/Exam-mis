...
if(isset($_GET['REF'])){
    ...
    // Fetch existing items for this consignment
    $existing_items_query = mysqli_query($conn, "SELECT cd.*, le.equipments_name, le.equipments_ModelNo, le.equipments_description 
                                               FROM consignment_details cd 
                                               LEFT JOIN laboratory_equipments le ON cd.consdetail_item = le.equipments_id 
                                               WHERE cd.consdetail_ref='$REF' AND cd.consdetail_status='Active'");
    $existing_items = [];
    while($item = mysqli_fetch_array($existing_items_query)) {
        $existing_items[] = $item;
    }
}
else{ 
...
<html>
<head>
    <title>Update Consignment Items</title>
    <style>
        .autocomplete {
            position: relative;
    ...
    newItem.innerHTML = `
        ...
        <div class="w-full md:w-2/2 px-3 mb-4">
            ...
            <div class="autocomplete">
                ...
                <input type="hidden" id="Consdetail_id_${itemCounter}" name="items[${itemCounter}][consdetail_id]" value="0">
            </div>
        </div>
...
function removeItem(rowIndex) {
    ...
    if (itemToRemove) {
        // Check if this is an existing item (has a consdetail_id > 0)
        const consdetailId = document.getElementById('Consdetail_id_' + rowIndex).value;
        if (consdetailId > 0) {
            // Mark for deletion in database
            document.getElementById('Consdetail_id_' + rowIndex).value = '-' + consdetailId;
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    for (let i = 1; i <= itemCounter; i++) {
        autocomplete(document.getElementById("Equipment_search_" + i), i);
        
        // Add event listeners for price calculations
        document.getElementById('Unity_Price_' + i).addEventListener('input', function() { calculatePrices(i); });
        document.getElementById('Quantity_' + i).addEventListener('input', function() { calculatePrices(i); });
    }
        ...
        <div class="w-full max-w-4xl">
            <div class="leading-loose">
                <form action="" method="POST" class="max-w-full m-4 p-10 bg-white rounded shadow-xl">
                    <p class="text-gray-800 font-medium text-center text-lg mb-6">Update Items in Consignment: <?php echo $consignment_number; ?></p>
                    ...
                    if(isset($_POST['Update'])){
                        ...
                        if(isset($_POST['items']) && is_array($_POST['items'])) {
                            foreach($_POST['items'] as $item) {
                                $Consdetail_id = mysqli_real_escape_string($conn, $item['consdetail_id']);   
                                ...
                                // Check if this item is marked for deletion (negative consdetail_id)
                                if ($Consdetail_id < 0) {
                                    $actualId = abs($Consdetail_id);
                                    $delete = mysqli_query($conn, "UPDATE consignment_details SET consdetail_status = 'Inactive' WHERE consdetail_id = '$actualId'");
                                    if($delete) {
                                        $deletedCount++;
                                    } else {
                                        $errorCount++;
                                ...
                                // Only process if equipment is selected and values are valid
                                else if (!empty($Equipment_id) && $Quantity > 0 && $Unity_Price >= 0) {
                                    // If consdetail_id > 0, this is an existing item to update
                                    if ($Consdetail_id > 0) {
                                        $update = mysqli_query($conn, "UPDATE consignment_details 
                                                                     SET consdetail_item = '$Equipment_id', 
                                                                         consdetail_quantity = '$Quantity', 
                                                                         consdetail_uprice = '$Last_price', 
                                                                         consdetail_tprice = '$Total_price'
                                                                     WHERE consdetail_id = '$Consdetail_id'");
                                    ...
                                    // If consdetail_id = 0, this is a new item to insert
                                    else {
                                        $insert = mysqli_query($conn,"INSERT INTO consignment_details
                                                                   (consdetail_id, consdetail_item, consdetail_ref, consdetail_quantity, consdetail_uprice, consdetail_tprice, consdetail_status) 
                                                                   VALUES 
                            ...
                            if($successCount > 0) {
                                echo '<div class="bg-green-300 mb-4 border border-green-500 text-green-800 px-4 py-3 rounded relative" role="alert">
                                        ...
                                        <span class="block sm:inline">' . $successCount . ' item(s) updated successfully!</span>
                                    </div>';
                            }
                            
                            if($deletedCount > 0) {
                                echo '<div class="bg-blue-300 mb-4 border border-blue-500 text-blue-800 px-4 py-3 rounded relative" role="alert">
                                        <strong class="font-bold">Notice:</strong>
                                        <span class="block sm:inline">' . $deletedCount . ' item(s) were removed.</span>
                                    </div>';
                            }
                            ...
                            if($errorCount > 0) {
                                echo '<div class="bg-red-300 mb-4 border border-red-500 text-red-800 px-4 py-3 rounded relative" role="alert">
                                        ...
                                        <span class="block sm:inline">' . $errorCount . ' item(s) failed to update. Please try again!</span>
                                    </div>';
                            }
                    ...
                    <div class="mb-6">
                        <button type="button" class="add-item-btn" onclick="addNewItem()">
                            <i class="fas fa-plus text-white mr-2"></i> Add New Item
                        </button>
                    </div>
                    
                    <div id="items-container">
                        <?php
                        // Generate item rows for existing items
                        if(count($existing_items) > 0) {
                            foreach($existing_items as $index => $item) {
                                $rowNumber = $index + 1;
                                $itemCounter = $rowNumber; // Update the JavaScript counter
                        ?>
                        <div class="item-row" id="item-row-<?php echo $rowNumber; ?>">
                            <div class="item-header">
                                <span class="item-number">Item #<?php echo $rowNumber; ?></span>
                                <button type="button" class="remove-item" onclick="removeItem(<?php echo $rowNumber; ?>)">Remove</button>
                            </div>
                            ...
                            <div class="w-full md:w-2/2 px-3 mb-4">
                                ...
                                <div class="autocomplete">
                                    <input id="Equipment_search_<?php echo $rowNumber; ?>" type="text" placeholder="Type to search for equipment by name, model number, or description..." 
                                           value="<?php echo htmlspecialchars($item['equipments_name']); ?>" 
                                           class="block appearance-none w-full bg-grey-200 border border-grey-200 text-grey-darker py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-grey" autocomplete="off" required>
                                    <input type="hidden" id="Equipment_id_<?php echo $rowNumber; ?>" name="items[<?php echo $rowNumber; ?>][equipment_id]" value="<?php echo $item['consdetail_item']; ?>" required>
                                    <input type="hidden" id="Consdetail_id_<?php echo $rowNumber; ?>" name="items[<?php echo $rowNumber; ?>][consdetail_id]" value="<?php echo $item['consdetail_id']; ?>">
                                </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm text-gray-600" for="Quantity_<?php echo $rowNumber; ?>">Quantity</label>
                                    <input class="w-full px-3 py-1 text-gray-700 bg-gray-200 rounded" id="Quantity_<?php echo $rowNumber; ?>" name="items[<?php echo $rowNumber; ?>][quantity]" 
                                           type="number" min="1" value="<?php echo $item['consdetail_quantity']; ?>" required>
                                </div>
                                
                                <div>
                                    <label class="block text-sm text-gray-600" for="Unity_Price_<?php echo $rowNumber; ?>">Unit Price on invoice</label>
                                    <input class="w-full px-3 py-1 text-gray-700 bg-gray-200 rounded" id="Unity_Price_<?php echo $rowNumber; ?>" name="items[<?php echo $rowNumber; ?>][unity_price]" 
                                           type="number" min="0" step="0.01" value="<?php echo number_format($item['consdetail_uprice'] / $consignment_apliedpercent, 2); ?>" required>
                                </div>
                                
                                <div>
                                    <label class="block text-sm text-gray-600" for="Last_price_<?php echo $rowNumber; ?>">Price after expenses (<?php echo $consignment_apliedpercent; ?>x)</label>
                                    <input class="w-full px-3 py-1 text-gray-700 bg-gray-200 rounded" id="Last_price_<?php echo $rowNumber; ?>" name="items[<?php echo $rowNumber; ?>][last_price]" 
                                           type="number" min="0" step="0.01" value="<?php echo $item['consdetail_uprice']; ?>" readonly>
                                </div>
                                
                                <div>
                                    <label class="block text-sm text-gray-600" for="Total_price_<?php echo $rowNumber; ?>">Total price</label>
                                    <input class="w-full px-3 py-1 text-gray-700 bg-gray-200 rounded" id="Total_price_<?php echo $rowNumber; ?>" name="items[<?php echo $rowNumber; ?>][total_price]" 
                                           type="number" min="0" step="0.01" value="<?php echo $item['consdetail_tprice']; ?>" readonly>
                                </div>
                            </div>
                        </div>
                        <?php
                            ...
                            // If no existing items, show one empty row
                            $itemCounter = 1;
                        ?>
                        <div class="item-row" id="item-row-1">
                            ...
                            <div class="w-full md:w-2/2 px-3 mb-4">
                                ...
                                <div class="autocomplete">
                                    ...
                                    <input type="hidden" id="Consdetail_id_1" name="items[1][consdetail_id]" value="0">
                                </div>
                                <p class="text-xs text-gray-500 mt-1">Search by equipment name, model number, or description</p>
                            </div>
                    ...
                    <center>
                        <div class="mt-6 flex gap-4">
                            <button type="submit" name="Update" class="btn px-6 py-2 text-white font-light tracking-wider bg-green-500 rounded hover:bg-green-600">
                                <i class="fas fa-save text-white mr-2"></i> Update All Items
                            </button>

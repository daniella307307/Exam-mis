    ...
    <div class="w-full max-w-lg">
        <div class="leading-loose">
            <form  action ="" method= "POST"  enctype="multipart/form-data" class="max-w-xl m-4 p-10 bg-white rounded shadow-xl">
                <p class="text-gray-800 font-medium">Add New User   Details</p>
      <?php
...
$find_detail = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM schools WHERE school_id='$school_ref'")); 
$school_region  = $find_detail['school_region'];
if(!empty($password)){
...
$update = mysqli_query($conn,"INSERT INTO users (user_id,firstname,lastname,email_address,phone_number,password,status,access_level, school_ref, user_country, user_region,user_image) VALUES 
                                                (NULL, '$firstname','$lastname','$email_address','$phone_number','$password','$status','$access_level','$school_ref','$user_country','$user_region','SAMPLE')");
 if($update){
	header('location:Users_registration'); 
...
?>
				<div class="">
				   ...
				   <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="cus_name"  name="firstname"   type="text"  placeholder="First Name" aria-label="Name">
                </div>
				<div class=""> 
                   <label class="block text-sm text-gray-600" for="cus_email">Last Name</label>
                    <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="cus_name"  name="lastname"  type="text"  placeholder="Last Name" aria-label="Name">
                </div>
				<div class="mt-2">
                    <label class="block text-sm text-gray-600" for="cus_email">Phone No</label>
                    <input class="w-full px-5  py-4 text-gray-700 bg-gray-200 rounded" id="cus_email" name="phone_number"   type="text"  placeholder="Phone No" aria-label="Email">
                </div>
                <div class="mt-2">
                    <label class="block text-sm text-gray-600" for="cus_email">Email</label>
                    <input class="w-full px-5  py-4 text-gray-700 bg-gray-200 rounded" id="cus_email" name="email_address"  type="text"  placeholder="Email adress" aria-label="Email" required>
   </div>
   <div class="mt-2">             

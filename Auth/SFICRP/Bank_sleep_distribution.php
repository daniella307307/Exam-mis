...
// Count total records
$query_student = "SELECT * FROM student_list WHERE student_school ='$school_ref' AND $action";
$count_result = mysqli_query($conn,$query_student );
            ...
            <div class="flex flex-1 flex-col md:flex-row lg:flex-row mx-2">
                <div class="mb-2 border-solid border-gray-300 rounded border shadow-sm w-full">
                    <div class="bg-gray-200 px-2 py-3 border-solid border-gray-200 border-b">
                         ...
                         <a href="Add_New_Student_on_banksleep?SLEEP_No=<?php echo $SLEEP_No;?>&STATUS=<?php echo $STATUS;?>"><button class='bg-blue-800 hover:bg-yellow-800 text-white font-bold py-2 px-4 rounded'><i class="fas fa-plus"></i>&nbsp;&nbsp;Add New student</button></a>
                         
                         <a href="Add_students_to_Bank_sleep?SLEEP_No=<?php echo $SLEEP_No;?>&STATUS=<?php echo $STATUS;?>"><button class='bg-blue-800 hover:bg-yellow-800 text-white font-bold py-2 px-4 rounded'><i class="fas fa-plus"></i> <i class="fas fa-users"></i>&nbsp;&nbsp;Add more students</button></a>
                        <big>
<?php
...
?>
                    ...
                    <div class="p-3">
                        <table class="table-responsive w-full rounded">
                            ...
                            <tbody>   
 <?php while($users_details = mysqli_fetch_array($count_result)) {
	 $student_id  =$users_details['student_id'];
   //$paid = $users_details['PAID'];
   $find_invoice = mysqli_fetch_array(mysqli_query($conn,"SELECT 
SUM(invoice_local) AS invoices,
SUM(pay_amount_local) AS paid,invoice_student FROM students_invoice  
LEFT  JOIN  invoice_payments  ON students_invoice.invoice_id = invoice_payments.pay_invoice
LEFT  JOIN student_list ON student_list.student_id= invoice_payments.payment_student WHERE payment_student !='' AND  invoice_student='$student_id'"));
   $invoices = $find_invoice['invoices'];
   $paid = $find_invoice['paid'];
   $invoice_student =$find_invoice['invoice_student'];

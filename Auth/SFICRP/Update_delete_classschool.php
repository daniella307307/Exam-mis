...
LEFT join school_levels ON school_classes.class_level = school_levels.level_id WHERE class_id='$ID' AND class_school='$school_ref'"));
    ...
    <div class="w-full max-w-lg">
        <div class="leading-loose">
            <form  action ="" method= "POST" class="max-w-xl m-4 p-10 bg-white rounded shadow-xl">
                <p class="text-gray-800 font-medium">Update  Class Name in <big><strong><?php echo $school_name;?></strong></p>
   <?php
   if($ACTION=="UPDATE"){
...
$select_class = mysqli_query($conn,"SELECT * FROM school_classes WHERE class_level='$Level_ref' AND class_name='$Class_Name' AND class_id!='$ID' AND class_school='$school_ref'");
...
$update = mysqli_query($conn,"UPDATE school_classes SET
class_name = '$Class_Name', 
class_level = '$Level_ref', 
class_school = '$school_ref',
class_country = '$user_country' WHERE  class_id ='$ID'");
 if($update){
...
?><div class="bg-green-300 mb-2 border border-green-300 text-white px-4 py-3 rounded relative" role="alert">
   ...
   $count_students = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM student_list WHERE student_class='$ID'")) ;
   if($count_students>0){
     	?><div class="bg-red-500 mb-2 border border-red-300 text-white px-4 py-3 rounded relative" role="alert">
                                    <strong class="font-bold">Failed This class has   <?php echo $count_students; ?>  students.  Can not be deleted .</strong>
  ...
  $delete = mysqli_query($conn,"DELETE FROM school_classes WHERE  class_id =$ID"); 
  $delete2 = mysqli_query($conn,"DELETE FROM school_class_rooms WHERE   room_class='$ID'");
  
  if($delete AND $delete2){
   ?><div class="bg-green-500 mb-2 border border-red-300 text-white px-4 py-3 rounded relative" role="alert">
                                    <strong class="font-bold">Class Deleted Successfully </strong>
                                    <span class="block sm:inline">Redirecting to the main Page </span>
                                    <span class="absolute top-0 top-0 right-0 px-4 py-3">
                                      <svg class="fill-current h-6 w-6 text-red" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><title>Close</title><path d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.698z"/></svg>
                                    </span>
   ...
   window.setTimeout(function(){
        ...
        window.location.href = "Level_Classes";
     ...
     <div class="bg-green-500 mb-2 border border-red-300 text-white px-4 py-3 rounded relative" role="alert">
                                    ...
                                    <span class="block sm:inline">Contact the Admin</span>
                                    <span class="absolute top-0 top-0 right-0 px-4 py-3">
                                      <svg class="fill-current h-6 w-6 text-red" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><title>Close</title><path d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.698z"/></svg>
                                    </span>
                                </div>
...
?>
        <div class=""> 
                        ...
                        <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="Class_Name" name="Class_Name"  value ="<?php echo $select_class['class_name'];?>" type="text" required >
                    </div>
                        ...
                        <div class="relative">
                            <select name="Level_ref"
                                ...
                                id="countries" required>
								 <option value="<?php echo $select_class['class_level'];?>"><?php echo $select_class['level_name'];?></option> 	
                ...
                if($ACTION=="UPDATE"){
                ?><div class="mt-4">
                    <center><button type="submit" name ="Update" class="px-4 py-1 text-white font-light tracking-wider bg-green-500 rounded" type="submit">Update Class  Details</button></center>
                </div> <?php     

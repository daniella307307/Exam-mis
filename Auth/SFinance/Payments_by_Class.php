...
$find_class123 = mysqli_query($conn,"SELECT * FROM school_classes
LEFT JOIN school_levels ON school_classes.class_level = school_levels.level_id
WHERE class_id='$CLASS_REF'");
$rows = mysqli_num_rows($find_class123);
if($rows>0){
 $class_details1 = mysqli_fetch_array($find_class123);  
 $level_name1 = $class_details1['level_name'];
$class_name1  = $class_details1['class_name'];
}else{
 $level_name1 = "Select";
$class_name1 = " Class";  
}
...
// Count total records
$count_query = "SELECT COUNT(*) FROM student_list WHERE student_class ='$CLASS_REF' AND student_school ='$school_ref' AND $action $search_query";
...
WHERE student_class ='$CLASS_REF' AND student_school ='$school_ref' AND $action $search_query 
...
<div class="flex flex-1">
    ...
    <main class="bg-white-500 flex-1 p-3 overflow-hidden">
        <div class="flex flex-col">
            ...
            <!--Grid Form-->
            <div class="flex flex-1 flex-col md:flex-row lg:flex-row mx-2">
                <div class="mb-2 border-solid border-gray-300 rounded border shadow-sp w-full">
                    <div class="bg-gray-200 px-2 py-3 border-solid border-gray-200 border-b">
                        ...
                        <a href="export_students_by_Class.php?CLASS=<?php echo $CLASS_REF; ?>&format=pdf"><button class='bg-blue-800 hover:bg-yellow-800 text-white font-bold py-2 px-4 rounded'><i class="fas fa-file-pdf"></i>&nbsp;&nbsp;PDF</button></a>
                        <a href="export_students_by_Class.php?CLASS=<?php echo $CLASS_REF;?>&format=excel"><button class='bg-blue-800 hover:bg-yellow-800 text-white font-bold py-2 px-4 rounded'><i class="fas fa-file-excel"></i>&nbsp;&nbsp;Excel</button></a>
                        
                        
                         <a href="Import_School_fee_Report.php?CLASS=<?php echo $CLASS_REF;?>&format=excel"><button class='bg-blue-800 hover:bg-yellow-800 text-white font-bold py-2 px-4 rounded'><i class="fas fa-file-excel"></i>&nbsp;&nbsp;Import School fee Report</button></a>
                        <br><br>
                        ...
                        <form method="GET" action="" class="inline-block ml-2">
                            ...
                            <div class="flex">
                                <select name="CLASS_REF"
                                    ...
                                    ?><option value="<?php echo $find_class['class_id']; ?>" <?php echo ($CLASS_REF == $find_class['class_id']) ? 'selected' : ''; ?>>
                                        <?php echo $find_class['level_name']; ?>/<?php echo $find_class['class_name']; ?>
                                    </option><?php	
                                    }
                                    ?>
                                </select>      
                    ...
                    <?php if($rows>0){
                    ?> 
                    <center><strong><h1>Students in :<?php echo $level_name1."/".$class_name1;?></h1></strong></center>
                    
                    <!-- Search Form Added Here -->
                    <div class="my-4 px-4">
                        <form method="GET" action="" class="inline-block">
                            <input type="hidden" name="CLASS_REF" value="<?php echo $CLASS_REF; ?>">
                            <input type="hidden" name="STATUS" value="<?php echo $STATUS; ?>">
                            <div class="flex items-center">
                                <input 
                                    ...
                                    class="border rounded-l py-2 px-4 w-64 focus:outline-none focus:border-blue-500"
                                >
                                <button 
                                    type="submit" 
                                ...
                                <?php if(isset($_GET['search'])): ?>
                                    <a 
                                        href="Students_Payments?CLASS_REF=<?php echo $CLASS_REF; ?>&STATUS=<?php echo $STATUS; ?>" 
                    ...
                    <?php } else {
                      ?>  <center><strong><h1> No class Selected Please Select a class</h1></strong></center><?php
                    }
                    ;?>
                                ...
                                <?php while($users_details = mysqli_fetch_array($select_user)) {
                                    ...
                                    <tr>
                                        ...
                                        <td class="border px-4 py-2">
                                            <a href="Payments_by_Class?STUDENT=<?php echo $users_details['student_id']; ?>&STATUS=<?php echo $STATUS;?>" class="bg-blue-600 cursor-pointer rounded p-1 mx-1 text-white"> &nbsp;<i class="fas fa-eye"></i></a>
                                        </td>
                                    </tr>
                                <?php } ?>
                            </tbody>
        ...
        <div class="flex justify-center">
            ...
            if ($total_pages > 1) {
                for ($i = 1; $i <= $total_pages; $i++) {
                    $url = "Payments_by_Class?page=$i&STATUS=$STATUS&CLASS_REF=$CLASS_REF";
                    if(isset($_GET['search'])) {
                        $url .= "&search=".urlencode($_GET['search']);
                    }
                    if ($i == $current_page) {
                        echo "<a href='$url' class='mx-1 p-2 bg-blue-500 text-white'>$i</a>";
                    } else {
                        echo "<a href='$url' class='mx-1 p-2 bg-gray-200'>$i</a>";
                    }
                }
            }
            ?>

...
if (isset($_GET['STATUS'])) {
    ...
    if ($STATUS == "Active") {
        $status = "Active";
        $action = "student_promotion!='$ID' AND student_status ='$status'";
    } else {
        $status = "Inactive";
        $action = "student_promotion!='$ID' AND(student_status !='Active' OR student_status ='$status')";
    }
} else {
    $STATUS = "Active";
    $status = $STATUS;
    $action = "student_promotion !='$ID' AND student_status ='Active'";
}
...
LEFT JOIN schools ON  student_list.student_region = schools.school_id  WHERE student_school ='$school_ref' AND $action LIMIT $offset, $records_per_page");
?>
            ...
            <div class="flex flex-1 flex-col md:flex-row lg:flex-row mx-2">
                <div class="mb-2 border-solid border-gray-300 rounded border shadow-sm w-full">
                    <div class="bg-gray-200 px-2 py-3 border-solid border-gray-200 border-b">
                         <a href="Assign_Students_in_promotion?ID=<?php echo $ID;?>&STATUS=Active"><button class='bg-green-500 hover:bg-blue-800 text-white font-bold py-2 px-4 rounded'>Active</button></a>
                        <a href="Assign_Students_in_promotion?ID=<?php echo $ID;?>&STATUS=Inactive"><button class='bg-red-500 hover:bg-yellow-800 text-white font-bold py-2 px-4 rounded'>Inactive</button></a>
                        ...
                        <table class="table-responsive w-full rounded">
                            <thead>
                                <tr>
                                    ...
                                    <th class="border w-1/12 px-4 py-2">Gender</th>  
                                    <th class="border w-1/6 px-4 py-2">Promotion</th> 
                            ...
                            <tbody>
                                <?php while($users_details = mysqli_fetch_array($select_user)) { ?>
                                    <tr>
                                        ...
                                        <td class="border px-4 py-2"><?php echo $users_details['certification_name']."/".$users_details['promotion_year']; ?></td>
                                        
                                        <td class="border px-4 py-2">
                                            <a href="Update_Student_Certificate?STUDENT=<?php echo $users_details['student_id']; ?>&ID=<?php echo $ID; ?>&STATUS=<?php echo $STATUS ;?>" class="bg-green-600 cursor-pointer rounded p-1 mx-1 text-white"><i class="fas fa-plus"></i> Add in Certification</a>
                                        </td>
                                    </tr>
                                <?php } ?>
                            </tbody>

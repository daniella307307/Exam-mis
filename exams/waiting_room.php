...
.badge{
  ...
  background:rgba(124,58,237,.25);border:1px solid rgba(168,85,247,.4);
  border-radius:20px;padding:5px 14px;font-size:12px;font-weight:700;
  color:#c084fc;text-transform:uppercase;letter-spacing:.8px;margin-bottom:16px;
}
.badge::before{
...
input[type=text]:focus,textarea:focus{
  border-color:#a855f7;background:rgba(124,58,237,.1);
}
...
<div class="card">
  ...
  <form method="POST">
    ...
    <span class="toggle-label">Are you playing as…</span>
    <div class="toggle-row">
      <button type="button" class="toggle-btn active" id="btnInd" onclick="setMode('individual')">
        👤 Individual
      </button>
      <button type="button" class="toggle-btn" id="btnGrp" onclick="setMode('group')">
        👥 Group
      </button>
    </div>
    ...
    <div class="field">
      ...
      <input type="text" name="school" placeholder="e.g. Kigali Secondary School" autocomplete="off"
             value="<?= htmlspecialchars($_POST['school'] ?? '') ?>">
    </div>

    <div class="field">
      <label class="lbl">Grade / Class</label>
      <input type="text" name="grade" placeholder="e.g. S4 PCB" autocomplete="off"
             value="<?= htmlspecialchars($_POST['grade'] ?? '') ?>">
    </div>

    <!-- Only shown in group mode -->
    <div class="group-fields" id="groupFields">
      <div class="field">
        <label class="lbl">Group Members (one per line)</label>
        <textarea name="members" rows="4"
          placeholder="Alice Uwase&#10;Bob Mugisha&#10;Claire Nzeyimana"><?= htmlspecialchars($_POST['members'] ?? '') ?></textarea>
      </div>
    </div>

    <button type="submit" name="save_group" class="btn-save">Save My Details</button>
  </form>
